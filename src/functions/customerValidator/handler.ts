...
import { isValidCpf, isValidCnpj, normalizeDocument } from './utils/documentValidator';
import { findUserByUsernameOrDocument, createUser } from './services/user.service';
...
const signUp: ValidatedEventAPIGatewayProxyEvent<typeof schema> = async (event) => {
  ...
  const existingUser = await findUserByUsernameOrDocument(username as string, normalizedDocument);
  ...
  const newUser = await createUser({
    ...
    legalNature: legalNature as string,
  });
